...
function updatingScore() {
  document
    ...
    .addEventListener("click", function (bubbleDetails) {
      let time = Number(document.querySelector(".timer").textContent)
      if (time === 0 || time === 60) incScore();
...
// bug
// score even updates before start game and after game over also because update score function is reacting on every click made on playground div

// bug resolved
// added a condition before updating score that only update when the timer is not 0 and 60